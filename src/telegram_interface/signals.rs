...
///   * `SIGUSR1` → weekly short SMART self-test on all drives, followed by a filesystem trim
...
pub(super) fn spawn(bot: Bot, admin_chat: ChatId) {
    tokio::spawn(async move {
        ...
        log::info!(
            "Signal-triggered maintenance enabled (SIGUSR1: short test + trim, SIGUSR2: scrub + long test)"
        );

        loop {
            tokio::select! {
                _ = sigusr1.recv() => {
                    log::info!("Received SIGUSR1: starting weekly maintenance");
                    super::handle_weekly_maintenance(&bot, admin_chat).await;
                }
                _ = sigusr2.recv() => {
